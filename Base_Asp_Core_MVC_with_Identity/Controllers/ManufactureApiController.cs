...
namespace MangagerBuyProduct.Controllers
{
    ...
    public class ManufactureApiController : ControllerBase
    {
        ...
        public ManufactureApiController(Base_Asp_Core_MVC_with_IdentityContext context, UserManager<UserSystemIdentity> uid)
        {
            _uid = uid;
            _context = context;
        }
        public IActionResult Index()
        {
            try
            {
                ...
                int recordsTotal = 0;
                var customerData = (from tempcustomer in _context.manufactures select tempcustomer);
                ...
                {
                    customerData = customerData.Where(m => m.ManufactureName.Contains(searchValue) || m.ManufactureCode.Contains(searchValue));
                }
