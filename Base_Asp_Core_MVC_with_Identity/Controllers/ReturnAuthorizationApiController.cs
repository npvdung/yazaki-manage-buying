    ...
    public class ReturnAuthorizationApiController : ControllerBase
    {
        ...
        public IActionResult Index()
        {
            try
            {
                ...
                var customerData = from tempcustomer in _context.returnAuthorizations
                                   join temp1 in _context.purchaseOrders on tempcustomer.PurchaseOrderId equals temp1.ID.ToString() into tempTable
                                   from leftJoinData in tempTable.DefaultIfEmpty()
                                   select new
                                   {
                                       // Chọn các trường từ bảng Childrens và OtherTable
                                       tempcustomer.ID,
                                       tempcustomer.ReturnAuthorizationCode,
                                       nameOrder = leftJoinData.PurchaseOrderCode,
                                       tempcustomer.AmountReturn
                                   };
                ...
                {
                    customerData = customerData.Where(m => m.ReturnAuthorizationCode.Contains(searchValue) || m.nameOrder.Contains(searchValue));
                }
