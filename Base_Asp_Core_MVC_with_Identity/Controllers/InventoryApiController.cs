...
namespace MangagerBuyProduct.Controllers
{
    ...
    public class InventoryApiController : ControllerBase
    {
        ...
        public InventoryApiController(Base_Asp_Core_MVC_with_IdentityContext context, UserManager<UserSystemIdentity> uid)
        {
            _uid = uid;
            _context = context;
        }
        public IActionResult Index()
        {
            try
            {
                ...
                int recordsTotal = 0;
                var customerData = (from tempcustomer in _context.inventory select tempcustomer);
                ...
                {
                    customerData = customerData.Where(m => m.InventoryCode.Contains(searchValue) || m.Name.Contains(searchValue));
                }
