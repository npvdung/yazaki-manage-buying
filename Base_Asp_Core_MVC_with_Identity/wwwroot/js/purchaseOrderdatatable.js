    ...
    columns: [
      { data: "id", name: "Id", autoWidth: true },
      {
        ...
        render: function (data, type, row) {
          ...
          return `<a href="/PurchaseOrder/Edit/${Id}" m-1">Xem | Sửa</a>`;
        },
      },
      {
        targets: 1,
        ...
        render: function (data, type, row) {
          ...
          if (status === 0) {
            return `<a href="/PurchaseOrder/Shipment/${Id}" m-1">Giao hàng</a>`;
          } else if (status === 1) {
            return `<a>Đang giao</a>`;
          } else if (status === 1) {
            return `<a>Hoàn thành</a>`;
          } else {
            return `<a>Từ chối</a>`;
          }
          return;
      ...
      },
      {
        ...
        render: function (data, type, row) {
          if (data === 0) {
            return '<span class="badge bg-warning">Chờ ship</span>';
          } else if (data === 1) {
            return '<span class="badge bg-info">Đang ship</span>';
          } else if (data === 2) {
